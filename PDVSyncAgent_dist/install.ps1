...
function Write-Step {
    ...
    Add-Content -Path $InstallLog -Value "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') | $msg" -ErrorAction SilentlyContinue
}
...
Add-Content -Path $InstallLog -Value "`n$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') | === INSTALL START ===" -ErrorAction SilentlyContinue
...
if ($sqlServices -and $sqlServices.Count -gt 0) {
    # Extract instance name from service name (MSSQL$HIPER -> HIPER)
...
try {
    ...
    # Script idempotente: pode rodar N vezes
    $tsqlLogin = @"
IF NOT EXISTS (SELECT 1 FROM sys.sql_logins WHERE name = '$DefaultSqlUser')
...
"@
    ...
    $tsqlUser = @"
IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = '$DefaultSqlUser')
...
try {
    ...
    if ($stores.Count -gt 0) {
        ...
        if ($stores.Count -eq 1) {
            ...
            $StoreName = if ($match) { $match.Nome } else { "PDV $StoreId" }
        }
    }
}
catch {
...
Add-Content -Path $InstallLog -Value "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') | === INSTALL COMPLETE ===" -ErrorAction SilentlyContinue
